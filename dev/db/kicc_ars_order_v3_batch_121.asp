...
<%
  ...
  Dim insertError, errorMessage, errorNumber, errorSource, failedOrderNo
  insertError = False
  errorMessage = ""
  errorNumber = 0
  errorSource = ""
  ...
  For i = 0 To orderCount - 1
    ...
    with adoRs
      ...
      .Fields("AUTO_INPUT")   = "0"  '// 자동입력 여부 (기본값: 0)
  ...
  If insertError Then
    ...
    '// 전체 실패로 변경 (응답 재생성) - 오류 상세 정보 포함
    Dim errorDetail
    errorDetail = "트랜잭션오류"
    If errorMessage <> "" Then
      errorDetail = errorDetail & " [" & errorNumber & "] " & errorMessage
      If errorSource <> "" Then
        errorDetail = errorDetail & " (" & errorSource & ")"
      End If
    End If
    If failedOrderNo <> "" Then
      errorDetail = errorDetail & " [주문번호:" & failedOrderNo & "]"
    End If
    ...
    For i = 0 To orderCount - 1
      orderResult = "{" & _
        ...
        """phone_no"":""" & JsonEncode(phone_no) & """," & _
        """result_code"":""0017""," & _
        """message"":""" & JsonEncode(errorDetail) & """" & _
        "}"
      resultArray(i) = orderResult
    Next
    successCount = 0
